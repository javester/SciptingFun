...
foreach ($c in $containers)
{
    ...
    {        
        ...
        Write-Output "`nGathering Blob info in Container '$($c.Name)'... Batch $loopc (of $maxblobs blobs each)"
        ...
        foreach ($b in $blobs)
        {
            Write-Output "Changing tier from '$oldtier' on blob $($b.ICloudBlob.Uri)"
